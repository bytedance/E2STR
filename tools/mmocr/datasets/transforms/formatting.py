...
@TRANSFORMS.register_module()
class PackTextRecogInputsWithPix2Struct(BaseTransform):
    ...
    def __init__(self,
        ...
        self.P2S_processor = Pix2StructImageProcessor(patch_size={'height': 4, 'width': 4}, max_patches=256)
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'img' in results:
            img = results['img']
            if len(img.shape) < 3:
                # np.stack((img,) * 3, axis=0)
                img = np.expand_dims(img, -1)
            
            #img = np.array(self.P2S_processor(img)['flattened_patches']).squeeze(0) # (256, 50)
            # import time
            # st = time.time()
            img = self.P2S_processor(img).permute(1, 0)
            if img.shape[0] < 50:
                img = torch.cat([img, torch.zeros((50-img.shape[0], 256))], dim=0)
                self.bad_data += 1
                print('bad_data: ', self.bad_data)
            elif img.shape[0] > 50:
                img = img[:50]
                self.bad_data += 1
                print('bad_data: ', self.bad_data)
            # ed = time.time()
            # print('img: ', img.shape, '  ', ed-st)
            # print('img: ', img.shape)
