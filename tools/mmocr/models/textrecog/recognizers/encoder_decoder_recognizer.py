...
class EncoderDecoderRecognizer(BaseRecognizer):
    ...
    def __init__(self,
        ...
        for param in self.decoder.parameters():
            sum_p += param.numel()
        print('sum param in decoder: ', sum_p)

    def extract_feat(self, inputs: torch.Tensor) -> torch.Tensor:
        """Directly extract features from the backbone."""
        # print('max: ', max(inputs[0].view(-1)).item())
        # print('min: ', min(inputs[0].view(-1)).item())
    ...
    def predict(self, inputs: torch.Tensor, data_samples: RecSampleList,
        ...
        if self.with_encoder:
            out_enc = self.encoder(feat, data_samples)
        res = self.decoder.predict(feat, out_enc, data_samples)
        # ed = time.time()
        # print((ed-st)/inputs.shape[0])
        return res
